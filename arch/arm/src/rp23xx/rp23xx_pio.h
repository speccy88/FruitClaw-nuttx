...
#ifndef __ARCH_ARM_SRC_RP23XX_RP23XX_PIO_H
#define __ARCH_ARM_SRC_RP23XX_RP23XX_PIO_H
...
#define RP23XX_PIO_NUM        3     /* Number of PIOs */
#define RP23XX_PIO_SM_NUM     4     /* Number of state machines per PIO */
...
#define check_pio_param(pio) valid_params_if(PIO, pio < RP23XX_PIO_NUM)
...
enum rp23xx_pio_fifo_join
  {
    RP23XX_PIO_FIFO_JOIN_NONE = 0,
    RP23XX_PIO_FIFO_JOIN_TX = 1,
    RP23XX_PIO_FIFO_JOIN_RX = 2,
    RP23XX_PIO_FIFO_JOIN_TXGET = 4,
    RP23XX_PIO_FIFO_JOIN_TXPUT = 8,
    RP23XX_PIO_FIFO_JOIN_PUTGET = 12
  };

/* MOV status types */

enum rp23xx_pio_mov_status_type
  {
    RP23XX_STATUS_TX_LESSTHAN = 0,
    RP23XX_STATUS_RX_LESSTHAN = 1,
    RP23XX_STATUS_IRQ_SET = 2
  };
...
typedef struct rp23xx_pio_program
  {
    const uint16_t *instructions;
    uint8_t length;
    int8_t origin; /* required instruction memory origin or -1 */
  }
rp23xx_pio_program_t;
...
/****************************************************************************
 * Name: rp23xx_sm_config_set_out_pins
 *
 * Description:
...
static inline void rp23xx_sm_config_set_out_pins(rp23xx_pio_sm_config *c,
                                                 uint32_t out_base,
                                                 uint32_t out_count)
{
  valid_params_if(PIO, out_base < 32);
  valid_params_if(PIO, out_count <= 32);
  c->pinctrl = (c->pinctrl & ~(RP23XX_PIO_SM_PINCTRL_OUT_BASE_MASK |
                               RP23XX_PIO_SM_PINCTRL_OUT_COUNT_MASK)) |
               (out_base << RP23XX_PIO_SM_PINCTRL_OUT_BASE_SHIFT) |
               (out_count << RP23XX_PIO_SM_PINCTRL_OUT_COUNT_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_set_pins
 *
 * Description:
...
static inline void rp23xx_sm_config_set_set_pins(rp23xx_pio_sm_config *c,
                                                 uint32_t set_base,
                                                 uint32_t set_count)
{
  valid_params_if(PIO, set_base < 32);
  valid_params_if(PIO, set_count <= 5);
  c->pinctrl = (c->pinctrl & ~(RP23XX_PIO_SM_PINCTRL_SET_BASE_MASK |
                               RP23XX_PIO_SM_PINCTRL_SET_COUNT_MASK)) |
               (set_base << RP23XX_PIO_SM_PINCTRL_SET_BASE_SHIFT) |
               (set_count << RP23XX_PIO_SM_PINCTRL_SET_COUNT_SHIFT);
}
...
static inline void rp23xx_sm_config_set_in_pins(rp23xx_pio_sm_config *c,
                                                uint32_t in_base)
{
  valid_params_if(PIO, in_base < 32);
  c->pinctrl = (c->pinctrl & ~RP23XX_PIO_SM_PINCTRL_IN_BASE_MASK) |
               (in_base << RP23XX_PIO_SM_PINCTRL_IN_BASE_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_sideset_pins
 *
 * Description:
...
static inline void rp23xx_sm_config_set_sideset_pins(rp23xx_pio_sm_config *c,
                                                     uint32_t sideset_base)
{
  valid_params_if(PIO, sideset_base < 32);
  c->pinctrl = (c->pinctrl & ~RP23XX_PIO_SM_PINCTRL_SIDESET_BASE_MASK) |
               (sideset_base << RP23XX_PIO_SM_PINCTRL_SIDESET_BASE_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_sideset
 *
 * Description:
...
static inline void rp23xx_sm_config_set_sideset(rp23xx_pio_sm_config *c,
                                                uint32_t bit_count,
                                                bool optional,
                                                bool pindirs)
{
  ...
  c->pinctrl = (c->pinctrl & ~RP23XX_PIO_SM_PINCTRL_SIDESET_COUNT_MASK) |
               (bit_count << RP23XX_PIO_SM_PINCTRL_SIDESET_COUNT_SHIFT);

  c->execctrl = (c->execctrl & ~(RP23XX_PIO_SM_EXECCTRL_SIDE_EN |
                                 RP23XX_PIO_SM_EXECCTRL_SIDE_PINDIR)) |
                (optional ? RP23XX_PIO_SM_EXECCTRL_SIDE_EN : 0) |
                (pindirs ? RP23XX_PIO_SM_EXECCTRL_SIDE_PINDIR : 0);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_clkdiv
 *
 * Description:
...
static inline void rp23xx_sm_config_set_clkdiv(rp23xx_pio_sm_config *c,
                                               float div)
{
  uint32_t div_int = (uint32_t)div;
  uint32_t div_frac = (uint32_t)((div - (float)div_int) * (1u << 8u));
  c->clkdiv = (div_frac << RP23XX_PIO_SM_CLKDIV_FRAC_SHIFT) |
              (div_int << RP23XX_PIO_SM_CLKDIV_INT_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_clkdiv_int_frac
 *
 * Description:
...
static inline void rp23xx_sm_config_set_clkdiv_int_frac(
                                            rp23xx_pio_sm_config *c,
                                            uint16_t div_int,
                                            uint8_t div_frac)
{
  c->clkdiv = (((uint32_t)div_frac) << RP23XX_PIO_SM_CLKDIV_FRAC_SHIFT) |
              (((uint32_t)div_int) << RP23XX_PIO_SM_CLKDIV_INT_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_wrap
 *
 * Description:
...
static inline void rp23xx_sm_config_set_wrap(rp23xx_pio_sm_config *c,
                                             uint32_t wrap_target,
                                             uint32_t wrap)
{
  valid_params_if(PIO, wrap < PIO_INSTRUCTION_COUNT);
  valid_params_if(PIO, wrap_target < PIO_INSTRUCTION_COUNT);
  c->execctrl = (c->execctrl & ~(RP23XX_PIO_SM_EXECCTRL_WRAP_TOP_MASK |
                                 RP23XX_PIO_SM_EXECCTRL_WRAP_BOTTOM_MASK)) |
                (wrap_target << RP23XX_PIO_SM_EXECCTRL_WRAP_BOTTOM_SHIFT) |
                (wrap << RP23XX_PIO_SM_EXECCTRL_WRAP_TOP_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_jmp_pin
 *
 * Description:
...
static inline void rp23xx_sm_config_set_jmp_pin(rp23xx_pio_sm_config *c,
                                                uint32_t pin)
{
  valid_params_if(PIO, pin < 32);
  c->execctrl = (c->execctrl & ~RP23XX_PIO_SM_EXECCTRL_JMP_PIN_MASK) |
                (pin << RP23XX_PIO_SM_EXECCTRL_JMP_PIN_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_in_shift
 *
 * Description:
...
static inline void rp23xx_sm_config_set_in_shift(rp23xx_pio_sm_config *c,
                                                 bool shift_right,
                                                 bool autopush,
                                                 uint32_t push_threshold)
{
  ...
  c->shiftctrl = (c->shiftctrl &
                  ~(RP23XX_PIO_SM_SHIFTCTRL_IN_SHIFTDIR |
                    RP23XX_PIO_SM_SHIFTCTRL_AUTOPUSH |
                    RP23XX_PIO_SM_SHIFTCTRL_PUSH_THRESH_MASK)) |
                 (shift_right ? RP23XX_PIO_SM_SHIFTCTRL_IN_SHIFTDIR : 0) |
                 (autopush ? RP23XX_PIO_SM_SHIFTCTRL_AUTOPUSH : 0) |
                 ((push_threshold & 0x1fu) <<
                  RP23XX_PIO_SM_SHIFTCTRL_PUSH_THRESH_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_out_shift
 *
 * Description:
...
static inline void rp23xx_sm_config_set_out_shift(rp23xx_pio_sm_config *c,
                                                  bool shift_right,
                                                  bool autopull,
                                                  uint32_t pull_threshold)
{
  ...
  c->shiftctrl = (c->shiftctrl &
                  ~(RP23XX_PIO_SM_SHIFTCTRL_OUT_SHIFTDIR |
                    RP23XX_PIO_SM_SHIFTCTRL_AUTOPULL |
                    RP23XX_PIO_SM_SHIFTCTRL_PULL_THRESH_MASK)) |
                 (shift_right ? RP23XX_PIO_SM_SHIFTCTRL_OUT_SHIFTDIR : 0) |
                 (autopull ? RP23XX_PIO_SM_SHIFTCTRL_AUTOPULL : 0) |
                 ((pull_threshold & 0x1fu) <<
                  RP23XX_PIO_SM_SHIFTCTRL_PULL_THRESH_SHIFT);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_fifo_join
 *
 * Description:
...
static inline void rp23xx_sm_config_set_fifo_join(rp23xx_pio_sm_config *c,
                                          enum rp23xx_pio_fifo_join join)
{
  valid_params_if(PIO, join >= RP23XX_PIO_FIFO_JOIN_NONE &&
                       join <= RP23XX_PIO_FIFO_JOIN_RX);
  c->shiftctrl = (c->shiftctrl &
                  (uint32_t)~(RP23XX_PIO_SM_SHIFTCTRL_FJOIN_TX |
                              RP23XX_PIO_SM_SHIFTCTRL_FJOIN_RX)) |
                 ((join == RP23XX_PIO_FIFO_JOIN_TX) ?
                  RP23XX_PIO_SM_SHIFTCTRL_FJOIN_TX : 0) |
                 ((join == RP23XX_PIO_FIFO_JOIN_RX) ?
                  RP23XX_PIO_SM_SHIFTCTRL_FJOIN_RX : 0);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_out_special
 *
 * Description:
...
static inline void rp23xx_sm_config_set_out_special(rp23xx_pio_sm_config *c,
                                                bool sticky,
                                                bool has_enable_pin,
                                                uint32_t enable_pin_index)
{
  c->execctrl = (c->execctrl &
                 (uint32_t)~(RP23XX_PIO_SM_EXECCTRL_OUT_STICKY |
                             RP23XX_PIO_SM_EXECCTRL_INLINE_OUT_EN |
                             RP23XX_PIO_SM_EXECCTRL_OUT_EN_SEL_MASK)) |
                (sticky ? RP23XX_PIO_SM_EXECCTRL_OUT_STICKY : 0) |
                (has_enable_pin ? RP23XX_PIO_SM_EXECCTRL_INLINE_OUT_EN : 0) |
                ((enable_pin_index <<
                  RP23XX_PIO_SM_EXECCTRL_OUT_EN_SEL_SHIFT)
                 & RP23XX_PIO_SM_EXECCTRL_OUT_EN_SEL_MASK);
}

/****************************************************************************
 * Name: rp23xx_sm_config_set_mov_status
 *
 * Description:
...
static inline void rp23xx_sm_config_set_mov_status(rp23xx_pio_sm_config *c,
                                  enum rp23xx_pio_mov_status_type status_sel,
                                  uint32_t status_n)
{
  valid_params_if(PIO, status_sel >= RP23XX_STATUS_TX_LESSTHAN &&
                       status_sel <= RP23XX_STATUS_RX_LESSTHAN);
  c->execctrl = (c->execctrl &
                 ~(RP23XX_PIO_SM_EXECCTRL_STATUS_SEL_MASK |
                   RP23XX_PIO_SM_EXECCTRL_STATUS_N_MASK)) |
                ((status_sel << RP23XX_PIO_SM_EXECCTRL_STATUS_SEL_SHIFT)
                 & RP23XX_PIO_SM_EXECCTRL_STATUS_SEL_MASK) |
                (status_n & RP23XX_PIO_SM_EXECCTRL_STATUS_N_MASK);
}

/****************************************************************************
 * Name: rp23xx_pio_get_default_sm_config
 *
 * Description:
...
static inline rp23xx_pio_sm_config rp23xx_pio_get_default_sm_config(void)
{
  rp23xx_pio_sm_config c =
    {
      0, 0, 0, 0
    };
  rp23xx_sm_config_set_clkdiv_int_frac(&c, 1, 0);
  rp23xx_sm_config_set_wrap(&c, 0, 31);
  rp23xx_sm_config_set_in_shift(&c, true, false, 32);
  rp23xx_sm_config_set_out_shift(&c, true, false, 32);
  return c;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_config
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_config(uint32_t pio,
                                uint32_t sm,
                                const rp23xx_pio_sm_config *config)
{
  check_pio_param(pio);
  check_sm_param(sm);
  putreg32(config->clkdiv, RP23XX_PIO_SM_CLKDIV(pio, sm));
  putreg32(config->execctrl, RP23XX_PIO_SM_EXECCTRL(pio, sm));
  putreg32(config->shiftctrl, RP23XX_PIO_SM_SHIFTCTRL(pio, sm));
  putreg32(config->pinctrl, RP23XX_PIO_SM_PINCTRL(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_get_index
 *
 * Description:
...
static inline uint32_t rp23xx_pio_get_index(uint32_t pio)
{
  check_pio_param(pio);
  return pio;
}
...
static inline uint32_t rp23xx_pio_get_dreq(uint32_t pio, uint32_t sm,
                                           bool is_tx)
{
  check_pio_param(pio);
  check_sm_param(sm);
...
/****************************************************************************
 * Name: rp23xx_pio_sm_set_enabled
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_enabled(uint32_t pio,
                                             uint32_t sm,
                                             bool enabled)
{
  check_pio_param(pio);
  check_sm_param(sm);
  putreg32((getreg32(RP23XX_PIO_CTRL(pio)) & ~(1u << sm)) |
           (enabled ? (1u << sm) : 0), RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_set_sm_mask_enabled
 *
 * Description:
 ...
 *
 * See Also:
 *   rp23xx_pio_enable_sm_mask_in_sync() if you wish to enable multiple
...
static inline void rp23xx_pio_set_sm_mask_enabled(uint32_t pio,
                                                  uint32_t mask,
                                                  bool enabled)
{
  check_pio_param(pio);
  putreg32((getreg32(RP23XX_PIO_CTRL(pio)) & ~mask) |
           (enabled ? mask : 0), RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_restart
 *
 * Description:
...
static inline void rp23xx_pio_sm_restart(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);
  setbits_reg32(1u << (RP23XX_PIO_CTRL_SM_RESTART_SHIFT + sm),
                RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_restart_sm_mask
 *
 * Description:
...
static inline void rp23xx_pio_restart_sm_mask(uint32_t pio, uint32_t mask)
{
  check_pio_param(pio);
  setbits_reg32((mask << RP23XX_PIO_CTRL_SM_RESTART_SHIFT) &
                RP23XX_PIO_CTRL_SM_RESTART_MASK,
                RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_clkdiv_restart
 *
 * Description:
 ...
 *   -- see rp23xx_pio_clkdiv_restart_sm_mask().
 *
 * Input Parameters:
 *   pio - PIO index (0..2)
 *   sm - State machine index (0..3)
...
static inline void rp23xx_pio_sm_clkdiv_restart(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);
  setbits_reg32(1u << (RP23XX_PIO_CTRL_CLKDIV_RESTART_SHIFT + sm),
                RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_clkdiv_restart_sm_mask
 *
 * Description:
...
static inline void rp23xx_pio_clkdiv_restart_sm_mask(uint32_t pio,
                                                     uint32_t mask)
{
  check_pio_param(pio);
  setbits_reg32((mask << RP23XX_PIO_CTRL_CLKDIV_RESTART_SHIFT) &
                RP23XX_PIO_CTRL_CLKDIV_RESTART_MASK,
                RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_enable_sm_mask_in_sync
 *
 * Description:
...
static inline void rp23xx_pio_enable_sm_mask_in_sync(uint32_t pio,
                                                     uint32_t mask)
{
  check_pio_param(pio);
  setbits_reg32((mask << (RP23XX_PIO_CTRL_CLKDIV_RESTART_SHIFT) &
                 RP23XX_PIO_CTRL_CLKDIV_RESTART_MASK) |
                (mask & RP23XX_PIO_CTRL_SM_ENABLE_MASK),
                RP23XX_PIO_CTRL(pio));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_get_pc
 *
 * Description:
...
static inline uint8_t rp23xx_pio_sm_get_pc(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);
  return (uint8_t)getreg32(RP23XX_PIO_SM_ADDR(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_exec
 *
 * Description:
 ...
 *
 * See Also:
 *   rp23xx_pio_sm_is_exec_stalled() to see if an executed instruction
...
inline static void rp23xx_pio_sm_exec(uint32_t pio, uint32_t sm,
                                      uint32_t instr)
{
  check_pio_param(pio);
  check_sm_param(sm);
  putreg32(instr, RP23XX_PIO_SM_INSTR(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_is_exec_stalled
 *
 * Description:
...
static inline bool rp23xx_pio_sm_is_exec_stalled(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);
  return getreg32(RP23XX_PIO_SM_EXECCTRL(pio, sm)) &
         RP23XX_PIO_SM_EXECCTRL_EXEC_STALLED ? true : false;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_exec_wait_blocking
 *
 * Description:
...
static inline void rp23xx_pio_sm_exec_wait_blocking(uint32_t pio,
                                                    uint32_t sm,
                                                    uint32_t instr)
{
  check_pio_param(pio);
  ...
  while (rp23xx_pio_sm_is_exec_stalled(pio, sm))
    ;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_wrap
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_wrap(uint32_t pio,
                                          uint32_t sm,
                                          uint32_t wrap_target,
                                          uint32_t wrap)
{
  ...
  putreg32((getreg32(RP23XX_PIO_SM_EXECCTRL(pio, sm)) &
            ~(RP23XX_PIO_SM_EXECCTRL_WRAP_TOP_MASK |
              RP23XX_PIO_SM_EXECCTRL_WRAP_BOTTOM_MASK)) |
           (wrap_target << RP23XX_PIO_SM_EXECCTRL_WRAP_BOTTOM_SHIFT) |
           (wrap << RP23XX_PIO_SM_EXECCTRL_WRAP_TOP_SHIFT),
           RP23XX_PIO_SM_EXECCTRL(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_out_pins
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_out_pins(uint32_t pio,
                                              uint32_t sm,
                                              uint32_t out_base,
                                              uint32_t out_count)
{
  ...
  putreg32((getreg32(RP23XX_PIO_SM_PINCTRL(pio, sm)) &
            ~(RP23XX_PIO_SM_PINCTRL_OUT_BASE_MASK |
              RP23XX_PIO_SM_PINCTRL_OUT_COUNT_MASK)) |
           (out_base << RP23XX_PIO_SM_PINCTRL_OUT_BASE_SHIFT) |
           (out_count << RP23XX_PIO_SM_PINCTRL_OUT_COUNT_SHIFT),
...
/****************************************************************************
 * Name: rp23xx_pio_sm_set_set_pins
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_set_pins(uint32_t pio,
                                              uint32_t sm,
                                              uint32_t set_base,
                                              uint32_t set_count)
{
  ...
  putreg32((getreg32(RP23XX_PIO_SM_PINCTRL(pio, sm)) &
            ~(RP23XX_PIO_SM_PINCTRL_SET_BASE_MASK |
              RP23XX_PIO_SM_PINCTRL_SET_COUNT_MASK)) |
           (set_base << RP23XX_PIO_SM_PINCTRL_SET_BASE_SHIFT) |
           (set_count << RP23XX_PIO_SM_PINCTRL_SET_COUNT_SHIFT),
           RP23XX_PIO_SM_PINCTRL(pio, sm));
...
/****************************************************************************
 * Name: rp23xx_pio_sm_set_in_pins
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_in_pins(uint32_t pio,
                                             uint32_t sm,
                                             uint32_t in_base)
{
  check_pio_param(pio);
  ...
  putreg32((getreg32(RP23XX_PIO_SM_PINCTRL(pio, sm)) &
            ~RP23XX_PIO_SM_PINCTRL_IN_BASE_MASK) |
           (in_base << RP23XX_PIO_SM_PINCTRL_IN_BASE_SHIFT),
           RP23XX_PIO_SM_PINCTRL(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_sideset_pins
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_sideset_pins(uint32_t pio,
                                                  uint32_t sm,
                                                  uint32_t sideset_base)
{
  check_pio_param(pio);
  ...
  putreg32((getreg32(RP23XX_PIO_SM_PINCTRL(pio, sm)) &
            ~RP23XX_PIO_SM_PINCTRL_SIDESET_BASE_MASK) |
           (sideset_base << RP23XX_PIO_SM_PINCTRL_SIDESET_BASE_SHIFT),
           RP23XX_PIO_SM_PINCTRL(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_put
 *
 * Description:
...
static inline void rp23xx_pio_sm_put(uint32_t pio, uint32_t sm,
                                     uint32_t data)
{
  check_pio_param(pio);
  check_sm_param(sm);

  putreg32(data, RP23XX_PIO_TXF(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_get
 *
 * Description:
...
static inline uint32_t rp23xx_pio_sm_get(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  return getreg32(RP23XX_PIO_RXF(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_is_rx_fifo_full
 *
 * Description:
...
static inline bool rp23xx_pio_sm_is_rx_fifo_full(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  return (getreg32(RP23XX_PIO_FSTAT(pio)) &
          (1u << (RP23XX_PIO_FSTAT_RXFULL_SHIFT + sm))) != 0;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_is_rx_fifo_empty
 *
 * Description:
...
static inline bool rp23xx_pio_sm_is_rx_fifo_empty(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  return (getreg32(RP23XX_PIO_FSTAT(pio)) &
          (1u << (RP23XX_PIO_FSTAT_RXEMPTY_SHIFT + sm))) != 0;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_get_rx_fifo_level
 *
 * Description:
...
static inline uint32_t rp23xx_pio_sm_get_rx_fifo_level(uint32_t pio,
                                                       uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  uint32_t bitoffs = RP23XX_PIO_FLEVEL_RX0_SHIFT +
                     sm * (RP23XX_PIO_FLEVEL_RX1_SHIFT -
                           RP23XX_PIO_FLEVEL_RX0_SHIFT);
  const uint32_t mask = RP23XX_PIO_FLEVEL_RX0_MASK >>
                        RP23XX_PIO_FLEVEL_RX0_SHIFT;
  return (getreg32(RP23XX_PIO_FLEVEL(pio)) >> bitoffs) & mask;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_is_tx_fifo_full
 *
 * Description:
...
static inline bool rp23xx_pio_sm_is_tx_fifo_full(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  return (getreg32(RP23XX_PIO_FSTAT(pio)) &
          (1u << (RP23XX_PIO_FSTAT_TXFULL_SHIFT + sm))) != 0;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_is_tx_fifo_empty
 *
 * Description:
...
static inline bool rp23xx_pio_sm_is_tx_fifo_empty(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  return (getreg32(RP23XX_PIO_FSTAT(pio)) &
          (1u << (RP23XX_PIO_FSTAT_TXEMPTY_SHIFT + sm))) != 0;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_get_tx_fifo_level
 *
 * Description:
...
static inline uint32_t rp23xx_pio_sm_get_tx_fifo_level(uint32_t pio,
                                                       uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  uint32_t bitoffs = RP23XX_PIO_FLEVEL_TX0_SHIFT +
                     sm * (RP23XX_PIO_FLEVEL_TX1_SHIFT -
                           RP23XX_PIO_FLEVEL_TX0_SHIFT);
  const uint32_t mask = RP23XX_PIO_FLEVEL_TX0_MASK >>
                        RP23XX_PIO_FLEVEL_TX0_SHIFT;
  return (getreg32(RP23XX_PIO_FLEVEL(pio)) >> bitoffs) & mask;
}

/****************************************************************************
 * Name: rp23xx_pio_sm_put_blocking
 *
 * Description:
...
static inline void rp23xx_pio_sm_put_blocking(uint32_t pio,
                                              uint32_t sm,
                                              uint32_t data)
{
  check_pio_param(pio);
  ...
  while (rp23xx_pio_sm_is_tx_fifo_full(pio, sm))
    ;
  rp23xx_pio_sm_put(pio, sm, data);
}

/****************************************************************************
 * Name: rp23xx_pio_sm_get_blocking
 *
 * Description:
...
static inline uint32_t rp23xx_pio_sm_get_blocking(uint32_t pio, uint32_t sm)
{
  check_pio_param(pio);
  check_sm_param(sm);

  while (rp23xx_pio_sm_is_rx_fifo_empty(pio, sm))
    ;
  return rp23xx_pio_sm_get(pio, sm);
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_clkdiv
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_clkdiv(uint32_t pio, uint32_t sm,
                                            float div)
{
  check_pio_param(pio);
  check_sm_param(sm);
  ...
  putreg32((div_frac << RP23XX_PIO_SM_CLKDIV_FRAC_SHIFT) |
           (div_int << RP23XX_PIO_SM_CLKDIV_INT_SHIFT),
           RP23XX_PIO_SM_CLKDIV(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_set_clkdiv_int_frac
 *
 * Description:
...
static inline void rp23xx_pio_sm_set_clkdiv_int_frac(uint32_t pio,
                                                     uint32_t sm,
                                                     uint16_t div_int,
                                                     uint8_t div_frac)
{
  ...
  putreg32((((uint32_t)div_frac) << RP23XX_PIO_SM_CLKDIV_FRAC_SHIFT) |
           (((uint32_t)div_int) << RP23XX_PIO_SM_CLKDIV_INT_SHIFT),
           RP23XX_PIO_SM_CLKDIV(pio, sm));
}

/****************************************************************************
 * Name: rp23xx_pio_sm_clear_fifos
 *
 * Description:
...
static inline void rp23xx_pio_sm_clear_fifos(uint32_t pio, uint32_t sm)
{
  ...
  xorbits_reg32(RP23XX_PIO_SM_SHIFTCTRL_FJOIN_RX,
                RP23XX_PIO_SM_SHIFTCTRL(pio, sm));
  xorbits_reg32(RP23XX_PIO_SM_SHIFTCTRL_FJOIN_RX,
                RP23XX_PIO_SM_SHIFTCTRL(pio, sm));
}
...
/****************************************************************************
 * Name: rp23xx_pio_can_add_program
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_can_add_program_at_offset
 *
 * Description:
 *   Determine whether the given program can (at the time of the call) be
...
/****************************************************************************
 * Name: rp23xx_pio_add_program
 *
 * Description:
 *   Attempt to load the program, panicking if not possible
 *
 * See Also:
 *   rp23xx_pio_can_add_program() if you need to check whether the program
 *   can be loaded
 *
 * Input Parameters:
 *   pio - PIO index (0..2)
...
/****************************************************************************
 * Name: rp23xx_pio_add_program_at_offset
 *
 * Description:
 ...
 *
 * See Also:
 *   rp23xx_pio_can_add_program_at_offset() if you need to check whether the
...
/****************************************************************************
 * Name: rp23xx_pio_remove_program
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_sm_drain_tx_fifo
 *
 * Description:
...
void rp23xx_pio_sm_set_pins(uint32_t pio, uint32_t sm, uint32_t pin_values);

/****************************************************************************
 * Name: rp23xx_pio_sm_set_pins_with_mask
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_sm_set_pindirs_with_mask
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_sm_set_consecutive_pindirs
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_claim_unused_sm
 *
 * Description:
...
/****************************************************************************
 * Name: rp23xx_pio_set_input_sync_bypass
 *
 * Description:
...
static inline void rp23xx_pio_set_input_sync_bypass(uint32_t pio,
                                                    uint32_t pin,
                                                    bool bypass)
{
  DEBUGASSERT(pin < RP23XX_GPIO_NUM);

  modbits_reg32(bypass ? (1 << pin) : 0,
                (1 << pin),
                RP23XX_PIO_INPUT_SYNC_BYPASS(pio));
}
...
#endif
#endif /* __ASSEMBLY__ */
#endif /* __ARCH_ARM_SRC_RP23XX_RP23XX_PIO_H */
