...
static void update_pixels(struct ws2812_dev_s  *dev_data)
{
  struct instance *priv       = (struct instance *)dev_data->private;
  clock_t          time_delta;
  DMA_HANDLE       dma_handle = rp23xx_dmachannel();
  dma_config_t     dma_config =
    {
      .dreq   = rp23xx_pio_get_dreq(priv->pio, priv->pio_sm, true),
      .size   = RP23XX_DMA_SIZE_WORD,
      .noincr = false
    };

  rp23xx_txdmasetup(dma_handle,
                    (uintptr_t) RP23XX_PIO_TXF(priv->pio, priv->pio_sm),
 ...
 *
 * Returned Value:
 *   A pointer to an internal structure used by rp23xx_ws2812
 *
 ****************************************************************************/
...
static int my_open(struct file *filep)
{
  ...
  if (priv->pixels == NULL)
    {
      lederr("rp23xx_ws2812 open: out of memory\n");
...
static int my_close(struct file *filep)
{
  ...
  ledinfo("rp23xx_ws2812 close dev: 0x%p\n", dev_data);

  priv->open_count -= 1;

  if (priv->open_count == 0  &&  priv->power_pin >= 0)
    {
      rp23xx_gpio_put(priv->power_pin, false);
    }
                        ...
                        size_t       len)
{
  ...
  ledinfo("rp23xx_ws2812 write dev: 0x%p\n", dev_data);
      ...
      for (xfer_index = 0; xfer_index < len; xfer_index += 4)
        {
          ...
          if (position >= (4 * dev_data->nleds))
            {
              ledinfo("rp23xx_ws2812 write off end: %d\n", position);
              break;
            }
  ...
  for (xfer_index = 0; xfer_index < len; xfer_index += 4)
    {
      ...
      if (position >= (4 * dev_data->nleds))
        {
          ledinfo("rp23xx_ws2812 read off end: %d\n", position);
          break;
        }
 ...
 *
 * Returned Value:
 *   An opaque pointer that can be passed to rp23xx_ws2812_teardown on
 *   success or NULL (with errno set) on failure
 ****************************************************************************/

void * rp23xx_ws2812_setup(const char *path,
                           int         port,
                           int         power_pin,
 ...
 *
 * Input Parameters:
 *   driver      - Opaque pointer returned by rp23xx_ws2812_setup.
 *
 * Returned Value:
...
int rp23xx_ws2812_release(void * driver)
{
  ...
  if (priv->open_count == 0)
    {
      ...
      rp23xx_pio_sm_set_enabled(priv->pio, priv->pio_sm, false);
      rp23xx_pio_sm_unclaim(priv->pio, priv->pio_sm);
