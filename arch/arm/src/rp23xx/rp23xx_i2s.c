/****************************************************************************
 * arch/arm/src/rp23xx/rp23xx_i2s.c
 *
...
#ifndef CONFIG_RP23XX_I2S_MAXINFLIGHT
...
#  undef CONFIG_RP23XX_I2S_DUMPBUFFERS
#endif
...
#ifndef CONFIG_RP23XX_I2S_DATALEN
...
#  define RP23XX_I2S_DATAMASK  1
#elif  CONFIG_RP23XX_I2S_DATALEN < 8 || CONFIG_RP23XX_I2S_DATALEN > 16
#  error Invalid value for CONFIG_RP23XX_I2S_DATALEN
#else
#  error Valid but supported value for CONFIG_RP23XX_I2S_DATALEN
#endif
...
struct rp23xx_i2s_s
{
  ...
  struct rp23xx_transport_s tx;     /* TX transport state */

  /* Pre-allocated pool of buffer containers */

  sem_t bufsem;                     /* Buffer wait semaphore */
  struct rp23xx_buffer_s *freelist; /* A list a free buffer containers */
  struct rp23xx_buffer_s containers[CONFIG_RP23XX_I2S_MAXINFLIGHT];
};
...
#ifdef CONFIG_RP23XX_I2S_DUMPBUFFERS
...
static struct rp23xx_buffer_s *
                i2s_buf_allocate(struct rp23xx_i2s_s *priv);
...
static int      i2s_checkwidth(struct rp23xx_i2s_s *priv, int bits);
...
static int      i2s_dma_flags(struct rp23xx_i2s_s *priv);
static int      i2s_dma_allocate(struct rp23xx_i2s_s *priv);
static void     i2s_dma_free(struct rp23xx_i2s_s *priv);
...
{
  .i2s_txchannels   = rp23xx_i2s_txchannels,
  .i2s_txsamplerate = rp23xx_i2s_txsamplerate,
  .i2s_txdatawidth  = rp23xx_i2s_txdatawidth,
  .i2s_send         = rp23xx_i2s_send,
  .i2s_ioctl        = rp23xx_i2s_ioctl,
};
...
static struct rp23xx_buffer_s *i2s_buf_allocate(struct rp23xx_i2s_s *priv)
{
  struct rp23xx_buffer_s *bfcontainer;
  irqstate_t flags;
  int ret;
...
static void i2s_buf_initialize(struct rp23xx_i2s_s *priv)
{
  ...
  priv->freelist = NULL;
  nxsem_init(&priv->bufsem, 0, CONFIG_RP23XX_I2S_MAXINFLIGHT);

  for (i = 0; i < CONFIG_RP23XX_I2S_MAXINFLIGHT; i++)
    {
...
static void i2s_txdma_timeout(wdparm_t arg)
{
  struct rp23xx_i2s_s *priv = (struct rp23xx_i2s_s *)arg;
...
static int i2s_txdma_setup(struct rp23xx_i2s_s *priv)
{
  ...
  bfcontainer = (struct rp23xx_buffer_s *)sq_remfirst(&priv->tx.pend);
  ...
  rp23xx_txdmasetup(priv->tx.dma,
                    rp23xx_i2s_pio_getdmaaddr(),
  ...
  rp23xx_dmastart(priv->tx.dma, i2s_txdma_callback, priv);
  ...
  while (sq_peek(&priv->tx.done) != NULL)
    {
      ...
      flags = enter_critical_section();
      bfcontainer = (struct rp23xx_buffer_s *)sq_remfirst(&priv->tx.done);
      leave_critical_section(flags);
  ...
  while (!sq_empty(&priv->tx.act))
    {
      ...
      bfcontainer = (struct rp23xx_buffer_s *)sq_remfirst(&priv->tx.act);
...
static int rp23xx_i2s_txchannels(struct i2s_dev_s *dev, uint8_t channels)
{
...
/****************************************************************************
 * Name: rp23xx_i2s_cleanup_queues
 *
 * Description:
...
static void i2s_cleanup_queues(struct rp23xx_i2s_s *priv)
{
  irqstate_t flags;
  struct rp23xx_buffer_s *bfcontainer;
      ...
      case AUDIOIOC_START:
        {
          ...
          if (priv->channels == 1)
            {
              if (priv->datalen == 16)
                mode = RP23XX_I2S_PIO_16BIT_MONO;
              else
                mode = RP23XX_I2S_PIO_8BIT_MONO;
            }
          else
            {
              if (priv->datalen == 16)
                mode = RP23XX_I2S_PIO_16BIT_STEREO;
              else
                mode = RP23XX_I2S_PIO_8BIT_STEREO;
           }

          rp23xx_i2s_pio_configure(mode, priv->samplerate);
...
static int i2s_dma_flags(struct rp23xx_i2s_s *priv)
{
  switch (priv->datalen)
    {
    case 8:
      priv->txconfig.size = RP23XX_DMA_SIZE_BYTE;
      break;

    case 16:
      priv->txconfig.size = RP23XX_DMA_SIZE_HALFWORD;
      break;
  ...
  priv->txconfig.dreq = rp23xx_i2s_pio_getdreq();
...
static void i2s_configure(struct rp23xx_i2s_s *priv)
{
  ...
  priv->datalen = CONFIG_RP23XX_I2S_DATALEN;
#ifdef CONFIG_DEBUG
  priv->align   = RP23XX_I2S_DATAMASK;
#endif
}
...
struct i2s_dev_s *rp23xx_i2sbus_initialize(int port)
{
  struct rp23xx_i2s_s *priv = NULL;
  irqstate_t flags;
  int ret;

  i2sinfo("port: %d\n", port);

  priv = kmm_zalloc(sizeof(struct rp23xx_i2s_s));
  if (!priv)
    {
